...
export class AssetManager {
  ...
  private loadSound(path: string): Promise<AudioBuffer> {
    return new Promise((resolve, reject) => {
      // Create a dummy audio buffer as fallback
      const createDummyBuffer = () => {
        console.warn(`Creating dummy audio buffer for: ${path}`);
        // Create a silent 1-second buffer at 44.1kHz
        const sampleRate = 44100;
        const buffer = this.audioLoader.context.createBuffer(2, sampleRate, sampleRate);
        resolve(buffer);
      };

      // Try to load the sound with fetch first
      fetch(path)
        .then(response => {
          if (!response.ok) {
        ...
        .then(arrayBuffer => {
          return this.audioLoader.context.decodeAudioData(arrayBuffer);
        })
        .then(audioBuffer => {
          resolve(audioBuffer);
        })
        .catch(error => {
          ...
          // Create a dummy buffer instead of failing
          createDummyBuffer();
        });
    });
  }
  ...
  /**
   * Register a procedural model that can be retrieved like a loaded model
   * @param name Name to identify the model
   * @param model The procedural model object
   */
  public registerProceduralModel(name: string, model: THREE.Object3D): void {
    this.models.set(name, model);
    console.log(`Registered procedural model: ${name}`);
  }
}
...
class AudioLoader {
  ...
  public load(
    ...
    const createSilentBuffer = () => {
      console.warn(`Creating silent buffer for: ${url}`);
      const sampleRate = this.context.sampleRate;
      const buffer = this.context.createBuffer(2, sampleRate, sampleRate);
      onLoad(buffer);
    };
    ...
    fetch(url)
      ...
      .then(arrayBuffer => {
        ...
        return this.context.decodeAudioData(arrayBuffer)
          .catch(error => {
            console.warn(`Audio decoding error for ${url}:`, error);
            throw error;
          });
      })
      .then(audioBuffer => {
      ...
      .catch(error => {
        ...
        // Create a silent buffer instead of failing completely
        createSilentBuffer();
